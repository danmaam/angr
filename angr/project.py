...
def load_trace(instruction_list, arch, **kwargs):
	# open file
...
class Project:
	...
	def __init__(self, thing,
		...
		if isinstance(thing, cle.Loader):
			if load_options:
				l.warning(
					"You provided CLE options to angr but you also provided a completed cle.Loader object!")
			self.loader = thing
			...
			self.loader = cle.Loader(
				self.filename, concrete_target=concrete_target, **load_options)

		# Step 2: determine its CPU architecture, ideally falling back to CLE's guess
		if isinstance(arch, str):
			# may raise ArchError, let the user see this
			self.arch = archinfo.arch_from_id(arch)
		...
		if self.concrete_target and self.arch.name not in ['X86', 'AMD64', 'ARMHF', 'MIPS32']:
			l.critical(
				"Concrete execution does not support yet the selected architecture. Aborting.")
			raise Exception("Incompatible options for the project")
		...
		# this is a flag for the convenience API, exec() and terminate_execution() below
		...
		self.is_java_jni_project = isinstance(
			self.arch, ArchSoot) and self.simos.is_javavm_with_jni_support
	...
	def _initialize_analyses_hub(self):
		...
		self.analyses.use_plugin_preset(
			self._analyses_preset if self._analyses_preset is not None else 'default')
	...
	def _register_object(self, obj, sim_proc_arch):
		...
		for lib_name in self.loader.missing_dependencies:
			try:
				...
				l.info(
					"There are no simprocedures for missing library %s :(", lib_name)
		...
		for reloc in obj.imports.values():
			...
			if not reloc.resolved:
				...
				if reloc.owner.guess_simprocs:
					...
					self._guess_simprocedure(
						func, reloc.owner.guess_simprocs_hint)
				else:
					l.debug("Ignoring unresolved import '%s' from %s ...?",
							func.name, reloc.owner)
				continue
			...
			# so we can get the calling convention as close to right as possible.
				...
				if self._check_user_blacklists(export.name):
					if not func.is_weak:
						l.info(
							"Using stub SimProcedure for unresolved %s from %s", func.name, sim_lib.name)
						self.hook_symbol(export.rebased_addr, sim_lib.get_stub(
							export.name, sim_proc_arch))
				else:
				...
				for sim_lib in missing_libs:
					if sim_lib.has_metadata(export.name):
						if self._check_user_blacklists(export.name):
							if not func.is_weak:
								l.info(
									"Using stub SimProcedure for unresolved %s from %s", export.name, sim_lib.name)
								self.hook_symbol(export.rebased_addr, sim_lib.get_stub(
									export.name, sim_proc_arch))
						else:
							l.info(
								"Using builtin SimProcedure for unresolved %s from %s", export.name, sim_lib.name)
							self.hook_symbol(export.rebased_addr, sim_lib.get(
								export.name, sim_proc_arch))
						break
				else:
					if not func.is_weak:
						l.info(
							"Using stub SimProcedure for unresolved %s", export.name)
			...
			# use the vanilla ReturnUnconstrained, assuming that this isn't a weak func
				...
				self.hook_symbol(export.rebased_addr, SIM_PROCEDURES['stubs']['ReturnUnconstrained'](
					display_name=export.name, is_stub=True))
	...
	def _guess_simprocedure(self, f, hint):
		...
		if hint == "win":
			hinted_libs = filter(lambda lib: lib if lib.endswith(
				".dll") else None, SIM_LIBRARIES)
		else:
			hinted_libs = filter(
				lambda lib: lib if ".so" in lib else None, SIM_LIBRARIES)
		...
		for lib in hinted_libs:
			if SIM_LIBRARIES[lib].has_implementation(f.name):
				...
				self.hook_symbol(
					hook_at, (SIM_LIBRARIES[lib].get(f.name, self.arch)))
				return True
	...
	def hook(self, addr, hook=None, length=0, kwargs=None, replace=False):
		...
		if isinstance(hook, type):
			raise TypeError(
				"Please instanciate your SimProcedure before hooking with it")

		if callable(hook):
			hook = SIM_PROCEDURES['stubs']['UserHook'](
				user_func=hook, length=length, **kwargs)
			...
			if sym is None:
				...
				for reloc in self.loader.find_relevant_relocations(symbol_name):
					if not reloc.symbol.is_weak:
						raise Exception(
							"Symbol is strong but we couldn't find its resolution? Report to @rhelmot.")
					if new_sym is None:
						new_sym = self.loader.extern_object.make_extern(
							symbol_name)
	...
	def is_symbol_hooked(self, symbol_name):
		...
		if sym is None:
			...
			return False
		hook_addr, _ = self.simos.prepare_function_symbol(
			symbol_name, basic_addr=sym.rebased_addr)
	...
	def __setstate__(self, s):
		...
		try:
			...
			l.warning(
				"Plugin preset %s does not exist any more. Fall back to the default preset.")
